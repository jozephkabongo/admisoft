<?php
    ...
    /**
     * Classe ProductManager représente le gestionnaire complet d'un produit 
     */
    class ProductManager {
        ...
        public function edit(array $data): bool {
            if (!isset($data['prod_id'])) {
                throw new InvalidArgumentException(message: "Identifiant du produit requis pour la mise à jour");
            }
            $this->db->beginTransaction();
            try {
                ...
                foreach ($requiredFields as $field) {
                    if (isset($data[$field])) {
                        switch ($data[$field]) {
                            case 'cat_id':
                                $this->db->update(table: 'tbl_products')->set(data: ['cat_id' => $data['cat_id']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;

                            case 'name':
                                $this->db->update(table: 'tbl_products')->set(data: ['p_name' => $data['name']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;

                            case 'price':
                                $this->db->update(table: 'tbl_products')->set(data: ['p_price' => $data['price']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;

                            case 'quantity':
                                $this->db->update(table: 'tbl_products')->set(data: ['p_qty' => $data['quantity']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;

                            case 'treshold':
                                $this->db->update(table: 'tbl_products')->set(data: ['p_treshold' => $data['treshold']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;

                            case 'image_path':
                                $this->db->update(table: 'tbl_products')->set(data: ['p_image' => $data['image_path']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;

                            case 'description': 
                                $this->db->update(table: 'tbl_products')->set(data: ['p_description' => $data['description']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;
                                
                            case 'feature':
                                $this->db->update(table: 'tbl_products')->set(data: ['p_feature' => $data['feature']])->where(condition: 'p_id = ?', params: [$data['prod_id']])->execute();
                                break;
                        }
                    } 
                }
        ...
        public function delete(int $product_id): bool {
            if (empty($product_id)) {
                throw new InvalidArgumentException(message: "Identifiant du produit requis");
            }
            $this->db->beginTransaction();
            try {
                $this->db->delete(table: 'tbl_products')->where(condition: 'p_id = ?', params: [$product_id])->execute();
                $this->db->commit();
                return true;
            } catch (QueryException $qe) {
                $this->db->rollBack();
         ...
         */
        public static function get(Database $db, int $product_id): ?array {
            if (empty($product_id)) {
        ...
        public static function getAll(Database $db): array {
            ...
            ->execute();
            return ProductManager::formateData(data: $result);
        }
