<?php
    ...
    class SchemaBuilder {
        ...
        public function build(string $table, Closure $blueprint): string {
            ...
            return match ($this->db->getDriver()) {
                'mysql' => "$sql ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
                'pgsql' => "$sql WITH (OIDS=FALSE)",
                default => $sql
            };
        }
    ...
    class TableSchema {
        ...
        public function decimal(string $name): self { 
            $this->currentColumn = "$name DECIMAL NOT NULL";
            $this->columns[] = $this->currentColumn;
            return $this;
        }
        ...
        public function enum(string $name, array $values, bool $nullable = false): self { 
            $driver = $this->driver;
            if ($driver === 'mysql') {
                $escapedValues = array_map(callback: fn($v): string => "'" . addslashes(string: $v) . "'", array: $values);
                $this->columns[] = "$name ENUM(" . implode(separator: ', ', array: $escapedValues) . ")";
            } else {
                $this->string($name);
                $allowed = implode(separator: '|', array: array_map(callback: 'preg_quote', array: $values));
                $this->constraints[] = "CHECK ($name ~ '^($allowed)$')";
            }

            if ($nullable) {
                $this->columns[count(value: $this->columns)-1] .= ' NULL';
                $this->constraints[] = "CHECK ($name IS NULL OR $name IN ('" . implode(separator: "','", array: $values) . "'))";
            } else {
                $this->columns[count(value: $this->columns)-1] .= ' NOT NULL';
            }
            return $this;
        }
        ...
        public function required(): self {
            $colIndex = count(value: $this->columns) - 1;
