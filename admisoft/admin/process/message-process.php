<?php
    ...
    if (isAdminLoggedIn()) {
        /**
         *  Processus du traitement des messages côté administrateur 
         */
            ...
            switch ($_SERVER['REQUEST_METHOD']) {
                case 'POST': 
                    ...
                    if (isset($data['receiver_id']) AND isset($data['message'])) {
                        if ($message->send(senderId: $_SESSION['admin']['id'], receiverId: $data['receiver_id'], content: sanitizeInput(input: $data['message']), attachement: $attachment['path'] ?? null) === true) {
                ...
                case 'GET':
                    ...
                    if (isset($_GET['id'])) {
                        $conversation = Message::getConversation(db: $database,user1: $_SESSION['admin']['id'], user2: $_GET['id']);
